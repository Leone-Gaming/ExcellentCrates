...
public class Cooldown {
    ...
    public static void cleanup(UUID p) {
        for (String k : cooldown.keySet()) {
            if (cooldown.get(k).containsKey(p)) {
                ((HashMap<?, ?>) cooldown.get(k)).remove(p);
            }
        }
    }
    ...
    // get cooldown in xx.xx format
    public static String getCooldownForPlayer(String k, Player p) {
        return String.format("%.1f", (double) ((Long) ((HashMap<?, ?>) cooldown.get(k)).get(p.getUniqueId())
                - System.currentTimeMillis()) / 1000);
    }
